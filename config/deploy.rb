...
#
set :stages, %w(production aws-staging)
set :default_stage, 'production'
require 'capistrano/ext/multistage'

set :scm, 'git'
set :repository,  "git@github.com:xinghao/sitemap_safe.git"
set :branch, "master"
...
namespace :deploy do
  ...
  task :pre_setup do
    sudo "[ -d /www/rails_apps/#{application} ] || #{sudo} mkdir /www/rails_apps/#{application}"
    #sudo "[ -d /SOLR/SolrServer ] || #{sudo} mkdir /SOLR/SolrServer"
    sudo "[ -d /www/rails_apps/#{application}/releases ] || #{sudo} mkdir /www/rails_apps/#{application}/releases"
    sudo "[ -d /www/rails_apps/#{application}/shared ] || #{sudo} mkdir /www/rails_apps/#{application}/shared"
    sudo "[ -d /www/rails_apps/#{application}/shared/cached-copy ] || #{sudo} mkdir /www/rails_apps/#{application}/shared/cached-copy"
    sudo "[ -d /www/rails_apps/#{application}/shared/log ] || #{sudo} mkdir /www/rails_apps/#{application}/shared/log"
    sudo "[ -d /www/rails_apps/#{application}/shared/pids ] || #{sudo} mkdir /www/rails_apps/#{application}/shared/pids"
    
    sudo "chown -R apache:apache /www/rails_apps/#{application}/shared/pids"
     sudo "chown -R apache:apache /www/rails_apps/#{application}/shared/log"
...
#    sudo "chown svc-rails.kazaaadm /SOLR"
  ...
  task :post_deploy do
     ...
     #run "export RAILS_ENV=#{rails_env} && cd #{latest_release} && rake assets:precompile"
     
#    sudo "rm -f #{latest_release}/Gemfile"
#    sudo "mv #{latest_release}/Gemfile_prod #{latest_release}/Gemfile"

    if mystage == "aws-staging"
      puts "AWS-Staging>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
    elsif mystage == "production"
      puts "Production>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
    end
    
    
    #run "export RAILS_ENV=#{rails_env} && cd #{latest_release} && bundle exec rake sitemap:refresh:no_ping"
